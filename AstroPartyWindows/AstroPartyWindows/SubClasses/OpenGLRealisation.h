...
protected:
	...
	// width / height
	float window_scale;
	Vec2D* cursor_current_position;
	Vec2D* cursor_last_position;
	Vec2D* cursor_press_position;
	Vec2D* cursor_release_position;
...
public:
	...
	StaticBuffer circle_buffer;
	StaticBuffer polygon_buffer;
	StaticBuffer rectangle_buffer;

	StaticBuffer button_horizontal_buffer;
	...
	// The flag actives then menu is moving by cursor.
	bool flag_move_menu;
...
#define CLEAR_FRAME_PERIOD	30;
	...
	// The shader use for draw annihilation area generator.
	...
	// The shader use for draw circles.
	...
	// The function draw map's circle.
	void DrawObject(const Map::Circle* circle, bool update_shader = false);
	...
	// The function draw current map.
	void DrawCurrentMap();
	// The function draw current menu.
	void DrawCurrentMenu();
	// The function draw indicated map.
	...
	const bool* game_p__flag_all_entities_initialisation;
	const bool* game_p__flag_round_results;

	const GameTypes::tic_t* game_p__global_timer;
	const GameTypes::tic_t* game_p__stunning_timer;
	...
	// Array of knives.
	const std::vector<Knife>* game_p__knives;
	// Array of lasers.
	...
	std::shared_mutex* game_p__knives_array_mtx;
