...
#define MAP_ORBIT_MAP__CIRCLES_COUNT	10
...
#define MAP_ORBIT_MAP__GRAVGEN_SAVE_BARRIER_RADIUS	(MAP_ORBIT_MAP__AREA_RADIUS / 20.0f)
#define MAP_ORBIT_MAP__GRAVGEN_KILL_BARRIER_RADIUS	(MAP_ORBIT_MAP__GRAVGEN_SAVE_BARRIER_RADIUS * 0.8f)
#define MAP_ORBIT_MAP__GRAVGEN_RADIUS				(MAP_ORBIT_MAP__GRAVGEN_SAVE_BARRIER_RADIUS * 0.5f)
...
#define EVENT0__EXTERNAL_PERIOD	5000 // 50 seconds
	...
	float in_radius =
		MAP_ORBIT_MAP__GRAVGEN_SAVE_BARRIER_RADIUS +
		sinf(
			(float)(global_timer % EVENT0__EXTERNAL_PERIOD) / (float)EVENT0__EXTERNAL_PERIOD * M_PI * 2.0f
		) * (MAP_ORBIT_MAP__GRAVGEN_SAVE_BARRIER_RADIUS - MAP_ORBIT_MAP__GRAVGEN_KILL_BARRIER_RADIUS);
	...
	map.circles_array[2].SetPosition(pos_2_3);
	map.circles_array[3].SetPosition(pos_2_3);
	map.circles_array[4].SetPosition(pos_4_5);
	map.circles_array[5].SetPosition(pos_4_5);
	map.circles_array[6].SetPosition(pos_6_7);
	map.circles_array[7].SetPosition(pos_6_7);
	map.circles_array[8].SetPosition(pos_8_9);
	map.circles_array[9].SetPosition(pos_8_9);

	map.circles_array[2].SetRadius(in_radius);
	map.circles_array[4].SetRadius(in_radius);
	map.circles_array[6].SetRadius(ex_radius);
	map.circles_array[8].SetRadius(ex_radius);
...
#define EVENT_1__TURRET_ANGULAR_VELOCITY	0.01f
#define EVENT_1__TURRET_DETECTION_DISTANCE	1.0f

void Game::Event1()
{
	if (turrets[0].exist)
	{
		float min_dist = EVENT_1__TURRET_DETECTION_DISTANCE;
	...
	CALCULATE_ANGLE:
		...
		if (min_id != PLAYERS_COUNT_T_MAX)
		{
			...
			turrets[0].Rotate(fmaxf(-EVENT_1__TURRET_ANGULAR_VELOCITY, fminf(-angle, EVENT_1__TURRET_ANGULAR_VELOCITY)));
		}
		else
		{
			turrets[0].Rotate(rotation_inverse ? -EVENT_1__TURRET_ANGULAR_VELOCITY : EVENT_1__TURRET_ANGULAR_VELOCITY);
		}
...
void Game::Event3()
{
	...
	{
		asteroids_array_mtx.unlock();
		Vec2F asteroid_velocities[4];
#define EVENT3_ASTEROID_VELOCITY		0.015f
#define EVENT3_ASTEROID_SPAWN_RADIUS	0.2f
		asteroid_velocities[0].Set(EVENT3_ASTEROID_VELOCITY, EVENT3_ASTEROID_VELOCITY);
		asteroid_velocities[1].Set(EVENT3_ASTEROID_VELOCITY, -EVENT3_ASTEROID_VELOCITY);
		asteroid_velocities[2].Set(-EVENT3_ASTEROID_VELOCITY, -EVENT3_ASTEROID_VELOCITY);
		asteroid_velocities[3].Set(-EVENT3_ASTEROID_VELOCITY, EVENT3_ASTEROID_VELOCITY);
		uint8_t dir = rand() % 4;
		asteroids_array_mtx.lock();
		AddEntity(Asteroid(
			-asteroid_velocities[dir].Normalize() * EVENT3_ASTEROID_SPAWN_RADIUS,
			asteroid_velocities[dir],
...
#define EVENT5__DOWN_RECTANGLE	3u
...
#define MAP_DESTRUCTIBLE__UNBREAKABLE_RECTANGLES_COUNT	4u
#define MAP_DESTRUCTIBLE__DESTROYABLE_RECTANGLES_COUNT	(12u * 9u)
#define MAP_DESTRUCTIBLE__RECTANGLES_COUNT				(MAP_DESTRUCTIBLE__UNBREAKABLE_RECTANGLES_COUNT + MAP_DESTRUCTIBLE__DESTROYABLE_RECTANGLES_COUNT + 1u)

#define MAP_DESTRUCTIBLE__FRAME_RECTANGLE_ID			0
#define MAP_DESTRUCTIBLE__UNBREAKABLE_RECTANGLE_ID		(MAP_DESTRUCTIBLE__FRAME_RECTANGLE_ID + 1u)
#define MAP_DESTRUCTIBLE__DESTROYABLE_RECTANGLE_ID		(MAP_DESTRUCTIBLE__UNBREAKABLE_RECTANGLE_ID + MAP_DESTRUCTIBLE__UNBREAKABLE_RECTANGLES_COUNT)
...
#define EVENT_10__LAST_POLYGON_ID	4
...
#define EVENT_10__ASTEROID_SPAWN_RADIUS	(EVENT_10__FRAME_SIZE * 0.4f)
	...
	for (size_t polygon = EVENT_10__FIRST_POLYGON_ID; polygon <= EVENT_10__LAST_POLYGON_ID; polygon++)
	{
...
#define MAP_COLLIDER__CENTER			6.0f
#define MAP_COLLIDER__FRAME_SIZE		4.0f

#define MAP_COLLIDER__INTERNAL_CIRCLE_RADIUS	(MAP_COLLIDER__FRAME_SIZE / 12.0f)
#define MAP_COLLIDER__EXTERNAL_CIRCLE_RADIUS	(MAP_COLLIDER__FRAME_SIZE / 5.0f)
#define MAP_COLLIDER__BARRIER_LENGTH	(MAP_COLLIDER__EXTERNAL_CIRCLE_RADIUS * 2.0f)
#define MAP_COLLIDER__BARRIER_SHIFT		(MAP_COLLIDER__FRAME_SIZE / 5.0f * 4.0f / 2.0f)

#define MAP_COLLIDER__RECTANGLES_COUNT	5u
#define MAP_COLLIDER__CIRCLES_COUNT	2u

#define MAP_COLLIDER__SPAWN_SHIFT		(MAP_COLLIDER__FRAME_SIZE * 0.9f / 2.0f)
...
void Game::Event11()
{
	...
	{
		Vec2F position((MAP_COLLIDER__INTERNAL_CIRCLE_RADIUS + MAP_COLLIDER__EXTERNAL_CIRCLE_RADIUS) / 2.0f, 0.0f);
		...
		AddEntity(Asteroid(
			Vec2F(MAP_COLLIDER__CENTER) + position,
			position.Perpendicular() / ((MAP_COLLIDER__INTERNAL_CIRCLE_RADIUS + MAP_COLLIDER__EXTERNAL_CIRCLE_RADIUS) / 2.0f) * 0.002f,
	...
	if (!((global_timer + 1) % EVENT_11__DESTROY_PERIOD))
	{
		...
		for (size_t id = 0, count = 0; count < bonuses_count; ++id)
		{
			if (bonuses[id].exist)
			{
				if (bonuses[id].GetDistance(Vec2F(MAP_COLLIDER__CENTER)) < MAP_COLLIDER__INTERNAL_CIRCLE_RADIUS)
				{
...
void CreateMap0_PlaceTwoCirclesAndGravGen(Game* game_object, std::vector<Map::Circle>& circles, size_t id, const Vec2F& position, float gravity) {
	circles[id].Set(
		position,
		MAP_ORBIT_MAP__GRAVGEN_KILL_BARRIER_RADIUS,
		MAP_PROPERTY_AGGRESSIVE | MAP_PROPERTY_KILLER | MAP_PROPERTY_UNBREAKABLE);
	circles[id + 1].Set(
		position,
		MAP_ORBIT_MAP__GRAVGEN_SAVE_BARRIER_RADIUS,
...
void Game::CreateMap0(std::vector<Vec2F>& ships_positions, std::vector<float>& ships_angles)
{
	...
	auto circles = std::vector<Map::Circle>(MAP_ORBIT_MAP__CIRCLES_COUNT);
	...
	CreateMap0_PlaceTwoCirclesAndGravGen(this, circles, 2, position, -MAP_ORBIT_MAP__GRAVGEN_FORCE);

	position -= Vec2F(0.0f, 2.0f * MAP_ORBIT_MAP__INTERNAL_ORBIT_RADIUS);
	CreateMap0_PlaceTwoCirclesAndGravGen(this, circles, 4, position, -MAP_ORBIT_MAP__GRAVGEN_FORCE);

	position = Vec2F(MAP_ORBIT_MAP__CENTER_POSITION) + Vec2F(MAP_ORBIT_MAP__EXTERNAL_ORBIT_RADIUS, 0.0f);
	CreateMap0_PlaceTwoCirclesAndGravGen(this, circles, 6, position, MAP_ORBIT_MAP__GRAVGEN_FORCE);

	position -= Vec2F(2.0f * MAP_ORBIT_MAP__EXTERNAL_ORBIT_RADIUS, 0.0f);
	CreateMap0_PlaceTwoCirclesAndGravGen(this, circles, 8, position, MAP_ORBIT_MAP__GRAVGEN_FORCE);
...
void Game::CreateMap1(std::vector<Vec2F>& ships_positions, std::vector<float>& ships_angles)
{
	...
	new_segment.Set(
		...
		Vec2F(MAP_TURRET_ON_CENTER__FRAME_SIZE + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		true);
	rectangles[MAP_TURRET_ON_CENTER__FRAME_RECTANGLE].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);

	new_segment.Set(
		Vec2F(-MAP_TURRET_ON_CENTER__CENTER_SQUARE_SIZE + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		Vec2F(MAP_TURRET_ON_CENTER__CENTER_SQUARE_SIZE + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		true);
	rectangles[MAP_TURRET_ON_CENTER__CENTER_RECTANGLE].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	new_segment.Set(
		...
		true);
	rectangles[MAP_TURRET_ON_CENTER__DOWN_LEFT_RECTANGLE].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	new_segment.Set(
		Vec2F(MAP_TURRET_ON_CENTER__RECTANGLE_IN_POSITION + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		Vec2F(MAP_TURRET_ON_CENTER__RECTANGLE_OUT_POSITION + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		true);
	rectangles[MAP_TURRET_ON_CENTER__UP_RIGHT_RECTANGLE].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	new_segment.Set(
		...
		true);
	rectangles[MAP_TURRET_ON_CENTER__UP_LEFT_RECTANGLE].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	new_segment.Set(
		Vec2F(MAP_TURRET_ON_CENTER__RECTANGLE_IN_POSITION + MAP_TURRET_ON_CENTER__CENTER_POSITION, -MAP_TURRET_ON_CENTER__RECTANGLE_IN_POSITION + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		Vec2F(MAP_TURRET_ON_CENTER__RECTANGLE_OUT_POSITION + MAP_TURRET_ON_CENTER__CENTER_POSITION, -MAP_TURRET_ON_CENTER__RECTANGLE_OUT_POSITION + MAP_TURRET_ON_CENTER__CENTER_POSITION),
		true);
	rectangles[MAP_TURRET_ON_CENTER__DOWN_RIGHT_RECTANGLE].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
...
#define MAP_GRAV_GEN__CENTER			MAP_GRAV_GEN__CENTER_POSITION
  ...
  auto circles = std::vector<Map::Circle>(MAP_GRAV_GEN__CIRCLES_COUNT);

	diagonal.Set(
		Vec2F(-MAP_GRAV_GEN__FRAME_SIZE + MAP_GRAV_GEN__CENTER_POSITION, -MAP_GRAV_GEN__FRAME_SIZE + MAP_GRAV_GEN__CENTER_POSITION),
		Vec2F(MAP_GRAV_GEN__FRAME_SIZE + MAP_GRAV_GEN__CENTER_POSITION, MAP_GRAV_GEN__FRAME_SIZE + MAP_GRAV_GEN__CENTER_POSITION),
		true);
	rectangles[0].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);
	...
	rectangles[1].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	diagonal.Set(
		Vec2F(MAP_GRAV_GEN__SQUARE_POSITION + MAP_GRAV_GEN__CENTER_POSITION, MAP_GRAV_GEN__SQUARE_POSITION + MAP_GRAV_GEN__CENTER_POSITION),
		Vec2F(-MAP_GRAV_GEN__SQUARE_SIZE + MAP_GRAV_GEN__CENTER_POSITION, -MAP_GRAV_GEN__SQUARE_SIZE + MAP_GRAV_GEN__CENTER_POSITION));
	rectangles[2].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	diagonal.Set(
		Vec2F(-MAP_GRAV_GEN__SQUARE_POSITION + MAP_GRAV_GEN__CENTER_POSITION, MAP_GRAV_GEN__SQUARE_POSITION + MAP_GRAV_GEN__CENTER_POSITION),
		Vec2F(MAP_GRAV_GEN__SQUARE_SIZE + MAP_GRAV_GEN__CENTER_POSITION, -MAP_GRAV_GEN__SQUARE_SIZE + MAP_GRAV_GEN__CENTER_POSITION));
	rectangles[3].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	diagonal.Set(
		Vec2F(MAP_GRAV_GEN__SQUARE_POSITION + MAP_GRAV_GEN__CENTER_POSITION, -MAP_GRAV_GEN__SQUARE_POSITION + MAP_GRAV_GEN__CENTER_POSITION),
		Vec2F(-MAP_GRAV_GEN__SQUARE_SIZE + MAP_GRAV_GEN__CENTER_POSITION, MAP_GRAV_GEN__SQUARE_SIZE + MAP_GRAV_GEN__CENTER_POSITION));
	rectangles[4].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);


	circles[0].Set(
		Vec2F(MAP_GRAV_GEN__CENTER + MAP_GRAV_GEN__CENTER_POSITION, MAP_GRAV_GEN__CENTER + MAP_GRAV_GEN__CENTER_POSITION),
		MAP_GRAV_GEN__CIRCLE_SIZE,
		MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_AGGRESSIVE);

	map.Set(rectangles, circles);
...
#define MAP_DECELERATION_AREA__FRAME_RECTANGLE_ID	0
#define MAP_DECELERATION_AREA__DOWN_RECTANGLE_ID	1
#define MAP_DECELERATION_AREA__RIGHT_RECTANGLE_ID	2
#define MAP_DECELERATION_AREA__UP_RECTANGLE_ID		3
#define MAP_DECELERATION_AREA__LEFT_RECTANGLE_ID	4
#define MAP_DECELERATION_AREA__RECTANGLES_COUNT		5
	...
	new_segment.Set(
		...
		Vec2F(MAP_DECELERATION_AREA__FRAME_SIZE, MAP_DECELERATION_AREA__FRAME_SIZE),
		true);
	rectangles[MAP_DECELERATION_AREA__FRAME_RECTANGLE_ID].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);
	...
	new_segment.Set(
		...
		true);
	rectangles[MAP_DECELERATION_AREA__DOWN_RECTANGLE_ID].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	new_segment.Set(
		...
		true);
	rectangles[MAP_DECELERATION_AREA__RIGHT_RECTANGLE_ID].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	new_segment.Set(
		Vec2F(MAP_DECELERATION_AREA__SIDE_SEGMENT_WIDTH, MAP_DECELERATION_AREA__OUT_SEGMENT_HIGH),
		Vec2F(-MAP_DECELERATION_AREA__SIDE_SEGMENT_WIDTH, MAP_DECELERATION_AREA__IN_SEGMENT_HIGH),
		true);
	rectangles[MAP_DECELERATION_AREA__UP_RECTANGLE_ID].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

	new_segment.Set(
		Vec2F(-MAP_DECELERATION_AREA__OUT_SEGMENT_HIGH, MAP_DECELERATION_AREA__SIDE_SEGMENT_WIDTH),
		Vec2F(-MAP_DECELERATION_AREA__IN_SEGMENT_HIGH, -MAP_DECELERATION_AREA__SIDE_SEGMENT_WIDTH),
		true);
	rectangles[MAP_DECELERATION_AREA__LEFT_RECTANGLE_ID].Set(new_segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
...
void Game::CreateMap4(std::vector<Vec2F>& ships_positions, std::vector<float>& ships_angles)
{
	...
	Segment diagonal = Segment(
		...
		Vec2F(MAP_MEGA_LASERS__CENTER_POSITION - MAP_MEGA_LASERS__FRAME_SIZE, MAP_MEGA_LASERS__CENTER_POSITION - MAP_MEGA_LASERS__FRAME_SIZE),
		true);
	rectangles[MAP_MEGA_LASERS__FRAME_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);
	...
	for (size_t i = MAP_MEGA_LASERS__SIDE_RECTANGLE; i < MAP_MEGA_LASERS__SIDE_RECTANGLE + MAP_MEGA_LASERS__LASER_RECTANGLES_COUNT; ++i)
	{
		diagonal.Set(
			...
			true);
		rectangles[i].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
		position1.PerpendicularThis();
		position2.PerpendicularThis();
	}
	...
	for (size_t i = MAP_MEGA_LASERS__LASER_RECTANGLE; i < MAP_MEGA_LASERS__LASER_RECTANGLE + MAP_MEGA_LASERS__LASER_RECTANGLES_COUNT; ++i)
	{
		diagonal.Set(
			...
			Vec2F(MAP_MEGA_LASERS__CENTER_POSITION) + position2,
			true);
		rectangles[i].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_AGGRESSIVE | MAP_PROPERTY_KILLER | MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
		position1.PerpendicularThis();
		position2.PerpendicularThis();
	}
...
#define MAP_DYNAMICAL__DOWN_RECTANGLE	EVENT5__DOWN_RECTANGLE
...
#define MAP_DYNAMICAL__CAMERA_SIZE	10.0f
	...
	const Segment& frame_diagonal = Segment(
		...
		Vec2F(
			...
			MAP_DYNAMICAL__CENTER_POSITION - MAP_DYNAMICAL__FRAME_SIZE),
		true);
	rectangles[MAP_DYNAMICAL__FRAME_RECTANGLE].Set(frame_diagonal, MAP_PROPERTY_COLLIDE_INSIDE | MAP_PROPERTY_UNBREAKABLE);
	...
	for (size_t i = MAP_DYNAMICAL__UP_RECTANGLE; i <= MAP_DYNAMICAL__LEFT_RECTANGLE; ++i)
	{
		rectangles[i].Set(
			...
			MAP_PROPERTY_COLLIDE_OUTSIDE | MAP_PROPERTY_UNBREAKABLE);
...
void Game::CreateMap6(std::vector<Vec2F>& ships_positions, std::vector<float>& ships_angles)
{
	...
	rectangles[MAP_DESTRUCTIBLE__FRAME_RECTANGLE_ID].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);

	Vec2F rectangle_position(MAP_DESTRUCTIBLE__FRAME_SIZE / 10.0f * 2.0f);
	for (size_t i = MAP_DESTRUCTIBLE__UNBREAKABLE_RECTANGLE_ID; i < MAP_DESTRUCTIBLE__DESTROYABLE_RECTANGLE_ID; ++i)
	{
		...
		rectangles[i].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	for (size_t group = 0; group < 4; ++group)
	{
		for (size_t position = 0; position < 3; ++position)
		{
			for (size_t rec = 0; rec < 9; ++rec)
			{
				...
				rectangles[MAP_DESTRUCTIBLE__DESTROYABLE_RECTANGLE_ID + group * 9 * 3 + position * 9 + rec].Set(
					diagonal,
					rec == 4 ? MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE :
					rec % 2 ? MAP_PROPERTY_AGGRESSIVE | MAP_PROPERTY_COLLIDE_OUTSIDE :
...
#define MAP_AGGRESSIVE__UP_LEFT_RECTANGLE		8u
	...
	Segment diagonal = Segment(
		...
		Vec2F(MAP_AGGRESSIVE__CENTER_POSITION + MAP_AGGRESSIVE__FRAME_SIZE),
		true);
	rectangles[MAP_AGGRESSIVE__FRAME_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL + MAP_AGGRESSIVE__ELEMENT_POSITION_Y + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__UP_RIGHT_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL + MAP_AGGRESSIVE__ELEMENT_POSITION_X + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__RIGHT_UP_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL - MAP_AGGRESSIVE__ELEMENT_POSITION_Y + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__RIGHT_DOWN_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL - MAP_AGGRESSIVE__ELEMENT_POSITION_X + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__DOWN_RIGHT_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL - MAP_AGGRESSIVE__ELEMENT_POSITION_Y + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__DOWN_LEFT_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL - MAP_AGGRESSIVE__ELEMENT_POSITION_X + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__LEFT_DOWN_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL + MAP_AGGRESSIVE__ELEMENT_POSITION_Y + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__LEFT_UP_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	diagonal = Segment(
		...
		Vec2F(
			...
			MAP_AGGRESSIVE__ELEMENT_DIAGONAL + MAP_AGGRESSIVE__ELEMENT_POSITION_X + MAP_AGGRESSIVE__CENTER_POSITION),
		true);
	rectangles[MAP_AGGRESSIVE__UP_LEFT_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_KILLER | MAP_PROPERTY_COLLIDE_OUTSIDE);

	map.Set(rectangles);

	/* Spawn entities */

	ships_positions[0].Set(MAP_AGGRESSIVE__CENTER_POSITION - 1.85f, MAP_AGGRESSIVE__CENTER_POSITION + 1.9f);
	ships_positions[1].Set(MAP_AGGRESSIVE__CENTER_POSITION + 1.9f, MAP_AGGRESSIVE__CENTER_POSITION + 1.85f);
	ships_positions[2].Set(MAP_AGGRESSIVE__CENTER_POSITION + 1.85f, MAP_AGGRESSIVE__CENTER_POSITION - 1.9f);
	ships_positions[3].Set(MAP_AGGRESSIVE__CENTER_POSITION - 1.9f, MAP_AGGRESSIVE__CENTER_POSITION - 1.85f);
	...
	camera.SetHightLimits(
		MAP_AGGRESSIVE__CENTER_POSITION - CAMERA_DEFAULT_HIGH_LIMITS,
		MAP_AGGRESSIVE__CENTER_POSITION - CAMERA_DEFAULT_HIGH_LIMITS,
		MAP_AGGRESSIVE__CENTER_POSITION + CAMERA_DEFAULT_HIGH_LIMITS,
		MAP_AGGRESSIVE__CENTER_POSITION + CAMERA_DEFAULT_HIGH_LIMITS);
	camera.SetPosition(Vec2F(MAP_AGGRESSIVE__CENTER_POSITION, MAP_AGGRESSIVE__CENTER_POSITION));
	camera.SetSize(MAP_DYNAMICAL__CAMERA_SIZE);
}
...
#define MAP_BROKEN__STAR_POLYGON__UP_LEFT_POINT		7u
	...
	Segment diagonal = Segment(
		...
		Vec2F(MAP_BROKEN__CENTER_POSITION - MAP_BROKEN__FRAME_SIZE),
		true);
	rectangles[MAP_BROKEN__FRAME_RECTANGLE].Set(diagonal, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);
	...
	polygons[MAP_BROKEN__UP_RIGHT_STAR_POLYGON].Set(
		...
		Vec2F(1.0f),
		points,
		MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_CLOSED);
	...
	ships_positions[0].Set(MAP_BROKEN__CENTER_POSITION - 1.9f, MAP_BROKEN__CENTER_POSITION + 1.85f);
	ships_positions[1].Set(MAP_BROKEN__CENTER_POSITION + 1.9f, MAP_BROKEN__CENTER_POSITION + 1.85f);
	ships_positions[2].Set(MAP_BROKEN__CENTER_POSITION + 1.85f, MAP_BROKEN__CENTER_POSITION - 1.9f);
	ships_positions[3].Set(MAP_BROKEN__CENTER_POSITION - 1.9f, MAP_BROKEN__CENTER_POSITION - 1.85f);
	...
	camera.SetHightLimits(
		MAP_BROKEN__CENTER_POSITION - MAP_BROKEN__FRAME_SIZE,
		MAP_BROKEN__CENTER_POSITION - MAP_BROKEN__FRAME_SIZE,
		MAP_BROKEN__CENTER_POSITION + MAP_BROKEN__FRAME_SIZE,
		MAP_BROKEN__CENTER_POSITION + MAP_BROKEN__FRAME_SIZE);
	camera.SetPosition(Vec2F(MAP_BROKEN__CENTER_POSITION, MAP_BROKEN__CENTER_POSITION));
	camera.SetSize(MAP_DYNAMICAL__CAMERA_SIZE);
}
...
#define MAP_PORTAL__SHORT_BARRIER_POSITION_X		0.6f
#define MAP_PORTAL__SHORT_BARRIER_LENGTH			0.9f
#define MAP_PORTAL__SHORT_BARRIER_WIDTH_2		0.1f
...
#define MAP_PORTAL__UP_RIGHT_BIG_RECTANGLE		0
#define MAP_PORTAL__DOWN_RIGHT_BIG_RECTANGLE	1
...
#define MAP_PORTAL__UP_LEFT_SHORT_RECTANGLE		11
	...
	Segment segment = Segment(
		...
		true);
	rectangles[MAP_PORTAL__UP_RIGHT_BIG_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);
	...
	segment.Set(
		...
		true);
	rectangles[MAP_PORTAL__DOWN_RIGHT_BIG_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);

	segment.Set(
		Vec2F(MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__FRAME_SIZE),
		Vec2F(MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__FRAME_SIZE + MAP_PORTAL__LOW_FRAME_SIZE),
		true);
	rectangles[MAP_PORTAL__DOWN_LEFT_BIG_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);

	segment.Set(
		Vec2F(MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__FRAME_SIZE, MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__FRAME_SIZE),
		Vec2F(MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__FRAME_SIZE + MAP_PORTAL__LOW_FRAME_SIZE, MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__FRAME_SIZE - MAP_PORTAL__LOW_FRAME_SIZE),
		true);
	rectangles[MAP_PORTAL__UP_LEFT_BIG_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);

	segment.Set(
		Vec2F(
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__SHORT_BARRIER_POSITION_X - MAP_PORTAL__SHORT_BARRIER_WIDTH_2,
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__FRAME_SIZE),
		Vec2F(
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__SHORT_BARRIER_POSITION_X + MAP_PORTAL__SHORT_BARRIER_WIDTH_2,
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__FRAME_SIZE - MAP_PORTAL__SHORT_BARRIER_LENGTH),
		true);
	rectangles[MAP_PORTAL__UP_RIGHT_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__SHORT_BARRIER_POSITION_X + MAP_PORTAL__SHORT_BARRIER_WIDTH_2),
		true);
	rectangles[MAP_PORTAL__RIGHT_UP_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__SHORT_BARRIER_POSITION_X - MAP_PORTAL__SHORT_BARRIER_WIDTH_2),
		true);
	rectangles[MAP_PORTAL__RIGHT_DOWN_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__FRAME_SIZE + MAP_PORTAL__SHORT_BARRIER_LENGTH),
		true);
	rectangles[MAP_PORTAL__DOWN_RIGHT_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__FRAME_SIZE + MAP_PORTAL__SHORT_BARRIER_LENGTH),
		true);
	rectangles[MAP_PORTAL__DOWN_LEFT_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION - MAP_PORTAL__SHORT_BARRIER_POSITION_X - MAP_PORTAL__SHORT_BARRIER_WIDTH_2),
		true);
	rectangles[MAP_PORTAL__LEFT_DOWN_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__SHORT_BARRIER_POSITION_X + MAP_PORTAL__SHORT_BARRIER_WIDTH_2),
		true);
	rectangles[MAP_PORTAL__LEFT_UP_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	segment.Set(
		...
		Vec2F(
			...
			MAP_PORTAL__CENTER_POSITION + MAP_PORTAL__FRAME_SIZE - MAP_PORTAL__SHORT_BARRIER_LENGTH),
		true);
	rectangles[MAP_PORTAL__UP_LEFT_SHORT_RECTANGLE].Set(segment, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	...
	ships_positions[0].Set(MAP_PORTAL__CENTER_POSITION - 1.05f, MAP_PORTAL__CENTER_POSITION + 1.1f);
	ships_positions[1].Set(MAP_PORTAL__CENTER_POSITION + 1.1f, MAP_PORTAL__CENTER_POSITION + 1.05f);
	ships_positions[2].Set(MAP_PORTAL__CENTER_POSITION + 1.05f, MAP_PORTAL__CENTER_POSITION - 1.1f);
	ships_positions[3].Set(MAP_PORTAL__CENTER_POSITION - 1.1f, MAP_PORTAL__CENTER_POSITION - 1.05f);
	...
	camera.SetHightLimits(
		MAP_PORTAL__CENTER_POSITION - CAMERA_DEFAULT_HIGH_LIMITS,
		MAP_PORTAL__CENTER_POSITION - CAMERA_DEFAULT_HIGH_LIMITS,
		MAP_PORTAL__CENTER_POSITION + CAMERA_DEFAULT_HIGH_LIMITS,
		MAP_PORTAL__CENTER_POSITION + CAMERA_DEFAULT_HIGH_LIMITS);
	camera.SetPosition(Vec2F(MAP_PORTAL__CENTER_POSITION, MAP_PORTAL__CENTER_POSITION));
	camera.SetSize(MAP_DYNAMICAL__CAMERA_SIZE);
}
...
#define MAP_NO_CENTER__FRAME_OUT_SIDE_LENGTH		(MAP_NO_CENTER__FRAME_SIZE * 0.4f)
#define MAP_NO_CENTER__FRAME_DISTANCE_TO_IN_SIDE	(MAP_NO_CENTER__FRAME_SIZE * 0.65f)

#define MAP_NO_CENTER__DISINTEGRATOR_UP		1
#define MAP_NO_CENTER__DISINTEGRATOR_RIGHT	2
#define MAP_NO_CENTER__DISINTEGRATOR_DOWN	3
#define MAP_NO_CENTER__DISINTEGRATOR_LEFT	4
#define MAP_NO_CENTER__DISINTEGRATORS_COUNT	(MAP_NO_CENTER__DISINTEGRATOR_LEFT - MAP_NO_CENTER__DISINTEGRATOR_UP + 1)
#define MAP_NO_CENTER__DISINTEGRATOR_POINTS_COUNT	3
#define MAP_NO_CENTER__DISINTEGRATOR_SHIFT		(MAP_NO_CENTER__FRAME_OUT_SIDE_LENGTH * 0.95f)
#define MAP_NO_CENTER_DISINTEGRATOR_MIN_SIZE	(EVENT_10__MIN_SIZE_COEF * MAP_NO_CENTER__FRAME_SIZE)
#define MAP_NO_CENTER_DISINTEGRATOR_MAX_SIZE	(EVENT_10__MAX_SIZE_COEF * MAP_NO_CENTER__FRAME_SIZE)
	...
	points[1].Set(MAP_NO_CENTER__FRAME_OUT_SIDE_LENGTH, 0.0f);
	points[2].Set(MAP_NO_CENTER__FRAME_DISTANCE_TO_IN_SIDE, MAP_NO_CENTER__FRAME_DISTANCE_TO_IN_SIDE - MAP_NO_CENTER__FRAME_OUT_SIDE_LENGTH);
	...
	points = std::vector<Vec2F>(MAP_NO_CENTER__DISINTEGRATOR_POINTS_COUNT);

	points[0].Set(0.0f, 0.0f);
	points[1].Set(1.0f, 0.0f);
	points[2].Set(0.0f, 1.0f);

	const auto& disintegrators_positions = std::vector<Vec2F>(
		{
			Vec2F(0.0f, MAP_NO_CENTER__DISINTEGRATOR_SHIFT),
			Vec2F(MAP_NO_CENTER__DISINTEGRATOR_SHIFT, 0.0f),
			Vec2F(0.0f, -MAP_NO_CENTER__DISINTEGRATOR_SHIFT),
			Vec2F(-MAP_NO_CENTER__DISINTEGRATOR_SHIFT, 0.0f)
		});

	for (size_t disintegrator = MAP_NO_CENTER__DISINTEGRATOR_UP; disintegrator <= MAP_NO_CENTER__DISINTEGRATOR_LEFT; disintegrator++)
	{
		polygons[disintegrator].Set(
			disintegrators_positions[disintegrator - MAP_NO_CENTER__DISINTEGRATOR_UP] + Vec2F(MAP_NO_CENTER__FRAME_POS),
			M_PI_4 - M_PI_2 * (disintegrator),
			Vec2F(MAP_NO_CENTER_DISINTEGRATOR_MAX_SIZE),
			points,
			MAP_PROPERTY_CLOSED | MAP_PROPERTY_AGGRESSIVE | MAP_PROPERTY_KILLER | MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);
	}
...
{
	auto rectangles = std::vector<Map::Rectangle>(MAP_COLLIDER__RECTANGLES_COUNT);
	rectangles[0].Set(
		Segment(Vec2F(MAP_COLLIDER__CENTER - MAP_COLLIDER__FRAME_SIZE / 2.0f), Vec2F(MAP_COLLIDER__FRAME_SIZE)),
		MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_INSIDE);

	Vec2F point1(MAP_COLLIDER__BARRIER_LENGTH / 2.0f, MAP_COLLIDER__BARRIER_SHIFT + 0.01f);
	Vec2F point2(-MAP_COLLIDER__BARRIER_LENGTH / 2.0f, MAP_COLLIDER__BARRIER_SHIFT - 0.01f);

	for (size_t i = 1; i < 5; ++i)
	{
		rectangles[i].Set(
			Segment(
				Vec2F(MAP_COLLIDER__CENTER) + point1,
				Vec2F(MAP_COLLIDER__CENTER) + point2,
				true),
			MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_COLLIDE_OUTSIDE);

		point1.PerpendicularThis();
		point2.PerpendicularThis();
	}

	auto circles = std::vector<Map::Circle>(MAP_COLLIDER__CIRCLES_COUNT);
	circles[0].Set(Vec2F(MAP_COLLIDER__CENTER), MAP_COLLIDER__EXTERNAL_CIRCLE_RADIUS, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_AGGRESSIVE);
	circles[1].Set(Vec2F(MAP_COLLIDER__CENTER), MAP_COLLIDER__INTERNAL_CIRCLE_RADIUS, MAP_PROPERTY_UNBREAKABLE, false);

	map.Set(rectangles, circles);

	AddEntity(DecelerationArea(Vec2F(MAP_COLLIDER__CENTER), -DEFAULT_FORCE_RESISTANCE_AIR_COEFFICIENT * 1.05f, MAP_COLLIDER__EXTERNAL_CIRCLE_RADIUS * 0.95f));
	AddEntity(GravGen(Vec2F(MAP_COLLIDER__CENTER), -GRAVITY_GENERATOR_DEFAULT_GRAVITY, MAP_COLLIDER__INTERNAL_CIRCLE_RADIUS * 0.9f));

	ships_positions[0].Set(MAP_COLLIDER__CENTER - MAP_COLLIDER__SPAWN_SHIFT, MAP_COLLIDER__CENTER + MAP_COLLIDER__SPAWN_SHIFT);
	ships_positions[1].Set(MAP_COLLIDER__CENTER + MAP_COLLIDER__SPAWN_SHIFT, MAP_COLLIDER__CENTER + MAP_COLLIDER__SPAWN_SHIFT);
	ships_positions[2].Set(MAP_COLLIDER__CENTER + MAP_COLLIDER__SPAWN_SHIFT, MAP_COLLIDER__CENTER - MAP_COLLIDER__SPAWN_SHIFT);
	ships_positions[3].Set(MAP_COLLIDER__CENTER - MAP_COLLIDER__SPAWN_SHIFT, MAP_COLLIDER__CENTER - MAP_COLLIDER__SPAWN_SHIFT);
	...
	camera.SetHightLimits(
		MAP_COLLIDER__CENTER - MAP_COLLIDER__FRAME_SIZE,
		MAP_COLLIDER__CENTER - MAP_COLLIDER__FRAME_SIZE,
		MAP_COLLIDER__CENTER + MAP_COLLIDER__FRAME_SIZE,
		MAP_COLLIDER__CENTER + MAP_COLLIDER__FRAME_SIZE);

	camera.SetPosition(Vec2F(MAP_COLLIDER__CENTER));
	camera.SetSize();
}
...
void Game::CreateMap12(std::vector<Vec2F>& ships_positions, std::vector<float>& ships_angles)
{
	...
	polygons[0].Set(Vec2F(MAP_KALEIDOSCOPE__CENTER), M_PI_4, Vec2F(MAP_KALEIDOSCOPE__EXTERNAL_CORNER_COEFFICIENT), points, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_CLOSED);
	polygons[1].Set(Vec2F(MAP_KALEIDOSCOPE__CENTER), 0.0f, Vec2F(MAP_KALEIDOSCOPE__INTERNAL_CORNER_COEFFICIENT), points, MAP_PROPERTY_UNBREAKABLE | MAP_PROPERTY_CLOSED);
