...
public:
	...
	GameTypes::tic_t stunning_timer;

	// Game status true = The game is on. false = The game stops.
	bool play_round;
	// Game status true = The game start next round.
	bool play_match;
	...
	// Index of array is ship's player number. Values of array are numbers of teams by every ship.
	// If player not play in current level, then teh value is zero.
	...
	// Controle is inverting
	bool rotation_inverse;
	// Buttons positions is swapped.
	...
	bool flag_all_entities_initialisation;
	// If true match will finish.
	bool flag_end_match;
	// If true results of round are drawing.
	...
	GameTypes::entities_count_t knives_count;
	...
	// Timer for check double clk for ships' burnout.
...
#ifdef _DEBUG
	logged_shared_mutex annih_area_gens_array_mtx = logged_shared_mutex("annih_area_gens_array_mtx");
	logged_shared_mutex asteroids_array_mtx = logged_shared_mutex("asteroids_array_mtx");
	logged_shared_mutex bombs_array_mtx = logged_shared_mutex("bombs_array_mtx");
	logged_shared_mutex bonuses_array_mtx = logged_shared_mutex("bonuses_array_mtx");
	logged_shared_mutex bullets_array_mtx = logged_shared_mutex("bullets_array_mtx");
	logged_shared_mutex camera_data_mtx = logged_shared_mutex("camera_data_mtx");
	logged_shared_mutex deceler_areas_array_mtx = logged_shared_mutex("deceler_areas_array_mtx");
	logged_shared_mutex dynamic_particles_array_mtx = logged_shared_mutex("dynamic_particles_array_mtx");
	logged_shared_mutex grav_gens_array_mtx = logged_shared_mutex("grav_gens_array_mtx");
	logged_shared_mutex input_values_mtx = logged_shared_mutex("input_values_mtx");
	logged_shared_mutex knives_array_mtx = logged_shared_mutex("knives_array_mtx");
	logged_shared_mutex lasers_array_mtx = logged_shared_mutex("lasers_array_mtx");
	logged_shared_mutex log_data_mtx = logged_shared_mutex("log_data_mtx");
	logged_shared_mutex map_data_mtx = logged_shared_mutex("map_data_mtx");
	logged_shared_mutex particles_array_mtx = logged_shared_mutex("particles_array_mtx");
	logged_shared_mutex pilots_array_mtx = logged_shared_mutex("pilots_array_mtx");
	logged_shared_mutex portals_array_mtx = logged_shared_mutex("portals_array_mtx");
	logged_shared_mutex ships_array_mtx = logged_shared_mutex("ships_array_mtx");
	logged_shared_mutex turrets_array_mtx = logged_shared_mutex("turrets_array_mtx");
#else // _DEBUG
	...
	std::shared_mutex knives_array_mtx;
...
// 		|---pause_menu
	...
	// Function adds entity to array that store entities of the same type.
	void AddEntity(const Laser& new_laser);
	...
	// Function adds entity to array that store entities of the same type.
	void AddEntity(const Turret& new_turret);
	...
	// The functions initializes all arrays this entities.
	Game();
	...
	// The functions initializes all arrays this maps.
	void InitMatch();

	// The function runs when the level initializes.
	// The function generates the map and spawning players.
	void InitLevel();

	// The function runs when the program starts.
	// The function creates all menus objects.
	void InitMenus();
	...
	pilot -> bomb -> log						pilot was destroyed by bomb
	...
	ship -> input_values ->
		laser -> bomb -> knife -> bullet		ship checks controlled values after that it shoots
	ship -> pilot -> log						ship kill pilot
	ship -> pilot								ship respawn pilot
	ship -> pilot -> bomb ->
					knife -> bonus -> log		ship was destroyed by bomb after that ship's knives destroyed and pilot and bonus spawn
	ship -> pilot -> knife -> bonus -> log		ship was destroyed by knife after that ship's knives destroyed and pilot and bonus spawn
	ship -> pilot -> bullet ->
	...
	*/
	...
	// t = n * k
	void KnivesDestroyAsteroids();

	// mtx: knife -> bullet
	// t = n * k
	void KnivesDestroyBullets();

	// mtx: knife -> map -> dynamic_particle
	// t = n * (m1 + m2 + m3)
 	void KnivesDestroyMap();

 	// mtx: knife -> turret -> dynamic_particle
	// t = n * k
 	void KnivesDestroyTurrets();
	...
	void PortalsTPEntities(std::vector<Entity_T>& entities, const GameTypes::entities_count_t exist_entities_count, std::shared_mutex& entity_array_mtx);
	...
	/*
	* In first phase entities influence to other entities
	after that change their angular velocities and forces
	but not changed linear velocities, positions and angles.
	* In second phase entities not influence to other entities,
	they convert their force and angular velocities values
	to new angle, velocity and position.
	*/

	/*
	The thread check map and match statuses.
	The thread can end or pause round.
	The thread update double click values.
	The thread set phases to default.
	The thread poll map's events.
	*/
	void PhysicThread0();
	// The thread calculate entities only.
	void PhysicThread1();
	// The thread calculate entities only.
	void PhysicThread2();
	// The thread calculate entities only.
	...
	// Destroy an asteroid
	// Divide it if this one enough big.
	...
	void DestroySupportEntitiesBy(ControlledEntity& producer);
	...
	void DebugLog__CheckMutexesLock();
	...
	The inventory will have a objects' types count that large than minimum objects' types count and
	less than maximum objects' types count.
	The inventory will have a objects count of every type large than minimum objects count and
	less than maximum objects count.
	*/
	...
	// open gl pointers
	OpenGL* object_p__open_gl_realization;
	// menu pointers
